...
UNZIP_TMPL  = 'unzip -p %s/%s%02d.zip'
...
# data set record -- ds_name: (table, do_gawk, do_sed, data_file)
# NOTE: if do_gawk is True, then "fmt_<ds_name>.awk" must exist in SCRIPT_DIR
DATA_SETS  = {
    'cn'     : ('cand',          False, True, None),
    'weball' : ('cand_fins',     True,  True, None),
    'cm'     : ('cmte',          False, True, None),
    'webk'   : ('cmte_fins',     True,  True, None),
    'pas2'   : ('cmte_contrib',  True,  True, None),
    'oth'    : ('cmte_misc',     True,  True, None),
    'ccl'    : ('cand_cmte',     False, True, None),
    'indiv'  : ('indiv_contrib', True,  True, 'itcont.txt')
}

def load_data_set(elec_year, ds_names, db_name):
    for ds_name in ds_names:
        (table, do_gawk, do_sed, data_file) = DATA_SETS[ds_name]
        unzip_cmd = UNZIP_TMPL % (DATA_DIR, ds_name, elec_year % 100)
        if data_file:
            unzip_cmd += " " + data_file
...
if __name__ == '__main__':
    def usage(msg):
        print("\nError: %s\n\nUsage: %s <data_set>[,...] <elec_year>[,...] <db_name>" % (msg, sys.argv[0]))
        sys.exit(1)
